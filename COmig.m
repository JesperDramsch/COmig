...
for v = vmin:dv:vmax;
    ...
    print('-dpng',sprintf('output/v%g.png',v));
    ...
    if v == vfinal % If loop reaches the correct velocity (estimated with constant velocity scan)
        ...
        dlmwrite('output/COGatherh0.dat',Kirchhoffdepth(:,:,1));
        dlmwrite('output/COGatherh250.dat',Kirchhoffdepth(:,:,2));
        dlmwrite('output/COGatherh500.dat',Kirchhoffdepth(:,:,3));
        dlmwrite('output/COGatherh750.dat',Kirchhoffdepth(:,:,4));
        dlmwrite('output/COGatherh1000.dat',Kirchhoffdepth(:,:,5));
...
axis ([0 75 0 1])
print('-dpng','output/freq_mig.png');                % Outputfile of figure
...
legend('Filtered data','Original data','Location','best')
set(gca,'Fontsize',24)
print('-dpng','output/wavelet_unnorm.png');
